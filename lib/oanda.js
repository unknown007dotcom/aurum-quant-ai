...
async function fetchMtfPayload(options = {}) {
  ...
  const outputsize = Math.max(30, Math.min(2000, Number.parseInt(String(options.outputsize || "200"), 10) || 200));
  const instrument = normalizeInstrument(options.instrument || config.instrument);

  const [entryData, h1Data, dailyData, weeklyData, monthlyData] = await Promise.all([
    fetchCandles({ ...config, instrument, timeframe: entryTf, count: outputsize }),
    ...
    fetchCandles({ ...config, instrument, timeframe: "1week", count: Math.min(outputsize, 500) }),
    fetchCandles({ ...config, instrument, timeframe: "1month", count: Math.min(outputsize, 240) }),
  ]);
  ...
  return {
    ...
    provider: "oanda",
    data: [
      { id: "entry", values: entryData, symbolUsed: instrument },
      ...
      { id: "benchmark", values: [], symbolUsed: "" },
